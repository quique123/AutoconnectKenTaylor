...
//    }///////////////////////
    ...
    if (doc.containsKey("thingspeakApiKey")) {
      strcpy(thingspeakApiKey, doc["thingspeakApiKey"]);      
    }
    
    if (doc.containsKey("sensorDht22")) {
      sensorDht22 = doc["sensorDht22"];
    }

    if (doc.containsKey("pinSda")) {
      pinSda = doc["pinSda"];
    }
    
    if (doc.containsKey("pinScl")) {
      pinScl = doc["pinScl"];
    }
  }
...
bool writeConfigFile() {
  ...
  doc["thingspeakApiKey"] = thingspeakApiKey;
  doc["sensorDht22"] = sensorDht22;
  doc["pinSda"] = pinSda;
  doc["pinScl"] = pinScl;
